...
## 5. Cấu trúc đầu ra (bám tài liệu mẫu MEETING-20177)

- **Header**: tiêu đề `BIÊN BẢN HỌP: MEETING SUMMARY` · **Mã cuộc họp** `MEETING-xxxxx` · **Ngày họp** · **Đối tượng** (người dự)
...
- **3. Danh sách công việc (Action Items)** — **bảng 4 cột**: Tên công việc · Người phụ trách · Trạng thái/Deadline · Ghi chú từ cuộc họp.
- **4. Phân công nhân sự (Personnel Assignments)** — **bảng 3 cột**: Thành viên · Tổ chức · Vai trò/Nhiệm vụ.
- **Footer**: "Biên bản này được tổng hợp tự động bởi {FOOTER_TEXT}".

**Quy ước thiếu dữ liệu:** ngày họp / người dự / phân công nhân sự nếu không có trong audio và người dùng không nhập → hiển thị **"Chưa xác nhận"** (giống mẫu).

> Mục "Vấn đề còn mở/rủi ro" (chọn ở bước brainstorm) được lồng vào Tóm tắt/Quyết định cho bám đúng 4 mục của mẫu. Có thể tách thành mục riêng sau nếu cần.
...
```json
{
  ...
  "action_items": [
    { "task": "tên công việc", "owner": "string|null", "status_deadline": "string|null", "note": "ghi chú từ cuộc họp" }
  ],
  "personnel": [
    { "member": "string", "organization": "string", "role": "string" }
  ]
}
```

**Prompt Gemini (tiếng Việt):** đóng vai thư ký biên bản; audio là tiếng Việt lẫn thuật ngữ Anh → **giữ nguyên thuật ngữ Anh**; chỉ trả JSON đúng schema; trường không xác định để `null` (không bịa).

**Hợp nhất metadata (trong `render.py`):** giá trị người dùng nhập **ghi đè** giá trị AI trích; cả hai trống → `"Chưa xác nhận"`. `meeting_id` luôn sinh phía server.
